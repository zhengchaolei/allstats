...
and dg.gameid = dp.gameid and dp.gameid = ga.id and gp.gameid = dg.gameid and gp.colour = dp.colour where name = '$username'";
		foreach ($dbHandle->query($sql, PDO::FETCH_ASSOC) as $row)
		{
			$score=$row["score"];
		}
...
$result = mysql_query("SELECT scores.score from scores where name= '$username'");
$row = mysql_fetch_array($result, MYSQL_ASSOC);
	$score=$row["score"];
